-- Mantener numero_remate oficial separado del nombre visible del remate.
-- Para remates de Tasaciones:
-- - `source_event_number` guarda el número oficial.
-- - `titulo` usa solo el nombre/descripcion, sin prefijo "Remate #....".
...
BEGIN
  ...
  v_titulo := NULLIF(trim(COALESCE(v_r.descripcion, '')), '');
...
-- Backfill visual: quitar prefijo numerado del título cuando proviene de Tasaciones.
UPDATE public.portal_remates pr
SET
  titulo = COALESCE(NULLIF(trim(COALESCE(r.descripcion, '')), ''), 'Sin nombre'),
  descripcion = NULLIF(trim(COALESCE(r.descripcion, '')), ''),
  source_event_number = r.numero_remate,
  updated_at = timezone('utc'::text, now())
FROM public.remates r
WHERE pr.tasaciones_remate_id = r.id
  AND pr.source_system = 'tasaciones';
